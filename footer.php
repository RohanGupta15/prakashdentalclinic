<script>
    function formCheck() {
        let fname = document.getElementById('fname').value;
        let lname = document.getElementById('lname').value;
        let email = document.getElementById('email').value;
        let phone = document.getElementById('phone').vlaue;
        let msg = document.getElementById('message').value;
        if (fname == '') {
            alert("First Name field is Empty!");
            document.getElementById('fname').focus();
            return false;
        } else if (lname == '') {
            alert("Last Name field is Empty!");
            document.getElementById('lname').focus();
            return false;
        } else if (email == '') {
            alert("Email field is Empty!");
            document.getElementById('email').focus();
            return false;
        } else if (phone == '') {
            alert("Phone Number field is Empty!");
            document.getElementById('phone').focus();
            return false;
        } else if (msg == '') {
            alert("Message field is Empty!");
            document.getElementById('message').focus();
            return false;
        }
    }
</script>

<?php
if (substr($_SERVER['PHP_SELF'], -13) != 'contactus.php') {
    echo '<div class="row mx-auto appoint justify-content-center align-items-center">
            <a href="" class="btn btn-lg btn-primary rounded" data-toggle="modal" data-target="#modalLoginAvatar">
                Request an Appointment!</a>
            </div>
            <div class="row maps mx-auto">
                <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3557.614129684333!2d80.95681631456584!3d26.915738766440402!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x399957bc1671d13d%3A0x3eb1ead88123fd0d!2sPRAKASH%20DENTAL%20CLINIC!5e0!3m2!1sen!2sin!4v1579867552831!5m2!1sen!2sin" width="100%" height="400px" frameborder="0" style="border:0;" allowfullscreen=""></iframe>
            </div>';
}
?>

<footer id="footer" class="footer-1">
    <div class="main-footer widgets-dark typo-light">
        <div class="container">
            <div class="row">
                <div class="col-xs-12 col-sm-6 col-md-3">
                    <div class="widget subscribe no-box">
                        <h5 class="widget-title">Prakash Dental Clinic<span></span></h5>
                        <p>
                            <i class="fa fa-map-marker"></i>
                            Sector-H ,2/2, Prakash Sadan<br />4 No. chauraha, Jankipuram, Lucknow,
                            <br>
                            <i class="fa fa-phone"></i>
                            7897345329 | 9451506015
                            <br>
                            <i class="fa fa-envelope"></i>
                            prakashdental32@gmail.com
                        </p>
                    </div>
                </div>

                <div class="col-xs-12 col-sm-6 col-md-3">
                    <div class="widget no-box">
                        <h5 class="widget-title">Quick Links<span></span></h5>
                        <ul class="thumbnail-widget">
                            <li>
                                <a href="index.php">HOME</a>
                            </li>
                            <li>
                                <a href="about.php">ABOUT</a>
                            </li>
                            <li>
                                <a href="gallery.php">GALLERY</a>
                            </li>
                            <li>
                                <a href="services.php">OUR SERVICES</a>
                            </li>
                            <li>
                                <a href="faq.php">FAQ's</a>
                            </li>
                            <li>
                                <a href="contactus.php">CONTACT US</a>
                            </li>
                        </ul>
                    </div>
                </div>

                <div class="col-xs-12 col-sm-6 col-md-3">
                    <div class="widget no-box">
                        <h5 class="widget-title">Timings<span></span></h5>
                        <p>
                            <i class="fa fa-calendar-check-o" style="font-size:25px; color: white"></i><br> <br>
                            <i class="fa fa-check" style="color: white"></i> Mor: 9:00 am to 2:00 pm <br>
                            <i class="fa fa-check" style="color: white"></i> Eve: 5:00 pm to 9:30 pm <br>
                            <i class="fa fa-close"></i> Friday Closing <br>
                        </p>
                        <a class="btn btn-warning" href="contactus.php">Register Now</a>
                    </div>
                </div>

                <div class="col-xs-12 col-sm-6 col-md-3">

                    <div class="widget no-box">
                        <h5 class="widget-title">Message Us</h5>

                        <p><a href="mailto:Prakashdental32@gmail.com" title="glorythemes" style="color: white"> <i class="fa fa-envelope-o" style="color: white"></i> prakashdental32@gmail.com</a></p>
                        <ul class="social-footer2">
                            <li class=""><a href="https://www.facebook.com/" target="_blank" title="Facebook"><img alt="Facebook" width="30" height="30" src="data:image/png;base64,"></a></li>
                            <li class=""><a href="https://twitter.com" target="_blank" title="Twitter"><img alt="Twitter" width="30" height="30" src="data:image/png;base64,"></a></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="footer-copyright">
        <div class="container">
            <div class="row">
                <div class="col-md-12 text-center">
                    <p>Copyright Prakash Dental Clinic © 2018. All rights reserved.</p>
                </div>
            </div>
        </div>
    </div>
</footer>
<script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAUZKU8U48D8fTUtb45k1NwGmGflTLeZYc&callback=initMap">
</script>
</body>

</html>